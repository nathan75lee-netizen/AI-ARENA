...
# [핵심] 8개 슬롯을 Gemini와 Groq의 최상위 모델로 재구성
# OpenRouter 없이도 GPT-4o급 성능을 내는 모델들로 배치했습니다.
PRIORITY_MAP = {
    "Gemini-Pro": [m for m in VALID_GEMINI if "pro" in m] + VALID_GEMINI,
    "Gemini-Flash": [m for m in VALID_GEMINI if "flash" in m] + VALID_GEMINI,
...
# [복구 엔진] Gemini와 Groq만 사용하여 무한 우회
def sync_api_call(family, model_id, prompt):
    ...
    for current_model in candidates:
        try:
            # 1. Gemini 계열 처리 (Gemini-Pro, Flash, Ref 등)
            if "gemini" in current_model.lower() or "Gemini" in family:
                m_name = current_model.split('/')[-1]
                model = genai.GenerativeModel(model_name=m_name)
                res = model.generate_content(prompt)
                if res and res.text: return res.text
            
            # 2. Groq 계열 처리 (Llama, Mixtral 등)
            else:
                ...
                elif r.status_code == 429: # 속도 제한 시 잠시 대기 후 다음 모델로
                    continue
        except: continue
    return f"⚠️ {family}: 호출 실패 (쿼터 초과 가능성)"
...
def main():
    ...
    st.markdown("<h2 style='text-align: center;'>⚡ AI Expert 8-Arena (v3.0.0)</h2>", unsafe_allow_html=True)
    
    with st.sidebar:
        st.write("### ⚙️ 모델 설정 (Gemini/Groq 전용)")
        ...
        if any(st.session_state.res_list):
            ...
            st.download_button("📥 결과 다운로드", data=report_text, file_name="arena_v3.md", use_container_width=True)

    main_q = st.text_area("Global Input", placeholder="Gemini와 Groq 최상위 모델이 동시에 분석합니다...", label_visibility="collapsed", key="g_input", height=100)
    ...
    if any(st.session_state.res_list) and st.button("📝 모든 답변 교차 검증 및 전문가 요약", use_container_width=True):
        ...
        if valid_ans:
            st.session_state.summary_res = sync_api_call("Gemini-Pro", "gemini-1.5-pro", f"결론 도출:\n\n{valid_ans}")
            st.rerun()
