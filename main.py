...
# [v2.7.0] v2.5.5의 안정적인 8개 계열 구조 + 클로드 답변 취합(Summarizer) 기능
@st.cache_resource
def setup_clients():
...
# v2.5.5의 8개 고정 계열 설정 (무료 모델 위주 배치)
MODEL_CONFIG = {
    "Gemini": VALID_GEMINI,
...
def apply_style():
    st.markdown("""
        ...
        .summary-btn button {
            background-color: #8b5cf6 !important; /* 요약 버튼은 보라색으로 차별화 */
...
def main():
    ...
    st.markdown("<h2 style='text-align: center; margin-bottom: 0;'>⚡ AI Expert Arena (v2.7.0)</h2>", unsafe_allow_html=True)
    ...
    # --- [NEW] 답변 취합 및 요약 섹션 ---
    st.divider()
    st.markdown("### ✨ 전문가 의견 종합 (Claude 요약)")
    
    st.markdown('<div class="summary-btn">', unsafe_allow_html=True)
    if st.button("📝 8개 답변 한눈에 요약하기", use_container_width=True):
        # 8개 답변 모으기
        combined_text = ""
        for i, name in enumerate(f_names):
            if st.session_state.res_list[i]:
                combined_text += f"[{name}의 답변]:\n{st.session_state.res_list[i]}\n\n"
        
        if combined_text.strip():
            with st.spinner("모든 의견을 취합하여 최적의 결론을 도출 중입니다..."):
                # 요약 전용 프롬프트
                sum_prompt = f"다음 8개 AI의 답변을 비교하여, 가장 정확하고 유용한 정보를 취합해 최종 결론을 요약해줘:\n\n{combined_text}"
                # 클로드 무료 모델(Haiku)을 사용하여 요약 실행
                summary = sync_api_call("Claude", selected["Claude"], sum_prompt)
                st.session_state.final_summary = summary
        else:
            st.warning("먼저 AI 답변을 생성해 주세요.")
    st.markdown('</div>', unsafe_allow_html=True)

    if 'final_summary' in st.session_state:
        st.markdown(f'''
            <div class="summary-card">
                <h4 style="margin-top:0; color:#15803d;">💡 클로드의 최종 브리핑</h4>
                <div style="line-height:1.6; color:#374151;">{st.session_state.final_summary}</div>
            </div>
        ''', unsafe_allow_html=True)
