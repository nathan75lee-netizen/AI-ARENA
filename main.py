...
# [v2.7.2] v2.5.5 기반 + 토큰 최적화(max_tokens 제한) 패치
@st.cache_resource
def setup_clients():
        ...
        if family == "Gemini":
            ...
            r = session.post("https://api.groq.com/openai/v1/chat/completions",
                ...
                json={
                    ...
                    "max_tokens": 2048  # [수정] 토큰 제한 추가
                }, timeout=20)
            return r.json()['choices'][0]['message']['content']
        else:
            # [수정] OpenRouter 호출 시 max_tokens를 명시하여 잔액 부족 에러 방지
            r = session.post("https://openrouter.ai/api/v1/chat/completions",
                ...
                json={
                    "model": model_id, 
                    "messages": [{"role": "user", "content": prompt}],
                    "max_tokens": 1500  # [핵심] 잔액이 적어도 호출되도록 낮게 설정
                }, timeout=45)
            data = r.json()
            if 'choices' in data:
                return data['choices'][0]['message']['content']
            else:
                return f"⚠️ {data.get('error', {}).get('message', 'API 응답 오류')}"
    except Exception as e:
        return f"⚠️ 연결 오류: {str(e)[:50]}"

# async_worker 및 main 함수는 v2.5.5와 동일하게 유지
